...
@max_score(0)
def test_each(test_name, simulator, width_p, delay_p):
    # This line must be first
    parameters = dict(locals())
    del parameters['test_name']
    del parameters['simulator']
    runner(simulator, timescale, tbpath, parameters, testname=test_name, pymodule="test_ramdelaybuffer")
...
@pytest.mark.parametrize("simulator", ["verilator", "icarus"])
@max_score(0.5)
def test_all(simulator, width_p, delay_p):
    # This line must be first
    parameters = dict(locals())
    del parameters['simulator']
    runner(simulator, timescale, tbpath, parameters, pymodule="test_ramdelaybuffer")

@pytest.mark.parametrize("width_p", [7])
@pytest.mark.parametrize("delay_p", [4])
@pytest.mark.parametrize("simulator", ["verilator"])
@max_score(.4)
def test_lint(simulator, width_p, delay_p):
    # This line must be first
    parameters = dict(locals())
    del parameters['simulator']
    lint(simulator, timescale, tbpath, parameters, pymodule="test_ramdelaybuffer")

@pytest.mark.parametrize("width_p", [7])
@pytest.mark.parametrize("delay_p", [4])
@pytest.mark.parametrize("simulator", ["verilator"])
@max_score(.1)
def test_style(simulator, width_p, delay_p):
    # This line must be first
    parameters = dict(locals())
    del parameters['simulator']
    lint(simulator, timescale, tbpath, parameters, compile_args=["--lint-only", "-Wwarn-style", "-Wno-lint"], pymodule="test_ramdelaybuffer")
...
class BufferModel():
    ...
    def produce(self):
        ...
        if(self._deqs >= self._dut.delay_p.value):
            ...
            assert int(got) == expected, f"Error! Output value on iteration {self._deqs} does not match expected. Expected: {expected}. Got: {got}"
        self._deqs += 1        
